...
@section('content')
        ...
        <div class="profile-headers" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
            ...
            <div class="profile-info">
                ...
                <div class="profile-stats">
                    ...
                    <!-- Скрыть личную статистику для других пользователей -->
                    @if(auth()->check() && auth()->id() == $user->id)
                        <div class="stat-item">
                            <span class="stat-number">{{ $user->likes()->count() }}</span>
        ...
        <div class="profile-content">
            <div class="profile-sidebar">
                <div class="sidebar-card">
                    ...
                    <ul class="nav-list">
                        <!-- Активная ссылка только на собственный профиль -->
                        @if(auth()->check() && auth()->id() == $user->id)
                            <li
                                class="nav-item {{ request()->is('profile') && (!isset($user) || (auth()->check() && $user->id == auth()->id())) ? 'active' : '' }}">
                        ...
                        @else
                            <li class="nav-item">
                                <!-- Предполагается маршрут для просмотра постов конкретного пользователя -->
                                <a href="{{ route('posts', $user->id) }}" class="nav-link">
                                    <i class="fas fa-book"></i> Посты пользователя
                                </a>
                            </li>
                        @endif
                    </ul>
                    ...
                    <div class="stats-list">
                        ...
                        @if(auth()->check() && auth()->id() == $user->id)
                            <div class="stat-row">
                                ...
                                <span class="stat-value">{{ $user->updated_at->format('d.m.Y H:i') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="profile-main">
                ...
                <!-- Отображать действия только для владельца профиля -->
                @if(auth()->check() && auth()->id() == $user->id)
                    <div class="profile-card">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <div class="actions-grid">
                                ...
                                <a href="{{ route('logout') }}" class="action-card"
    ...
    <style>
        ...
        .avatar-link::after {
            content: "\f303";
            /* Unicode для иконки карандаша (fa-pencil-alt) */
        ...
        .text-muted {
            color: #adb5bd;
            /* Серый цвет для скрытой информации */
        }
